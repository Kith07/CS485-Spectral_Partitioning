...
int set_Partitions(int s_id, int num_procs, int s) {
...
void recursive_bisection(vector<vector<float>>& eigen_matrix, vector<int> nodes, int depth, int max_depth, int s, int s_id, int g_size, int g_rank, MPI_Comm comm, vector<vector<int>>& clusters) {
    ...
    if (depth == max_depth || nodes.size() <= 1) {
        if (g_rank == set_Partitions(s_id, g_size, s)) {
            clusters[s_id] = nodes;
        }
        return;
    }
...
void get_partitions(vector<vector<float>>& eigen_matrix, vector<int>& node_ids, int n_parts, int rank, int size, MPI_Comm comm, vector<vector<int>>& parts) {
    ...
    vector<int> local_ids(node_ids.begin() + start, node_ids.begin() + start + count);

    /* compute centroid of local nodes */
    vector<float> local_sum(k, 0.0f);
    for (int idx : local_ids)
        for (int j = 0; j < k; j++)
    ...
    vector<vector<float>> R = inertial_matrix(eigen_matrix, local_ids, centroid);
...
int main(int argc, char** argv) {
    ...
    if (g_rank == 0) {
        ...
        /*REMEMBER: allocate and feed matrix in column-major format*/
        raw.resize(n * n);
        adj_matrix(fname, raw.data(), n);

        /* call the CUDA eigensolver func*/
        start1 = clock();
    ...
    /*TODO: time the partitioning*/
    start2 = clock();       
    get_partitions(eigen_matrix, nodes, n_parts, g_rank, g_size, MPI_COMM_WORLD, parts);
    end2 = clock();

    /*TODO: barrier to avoid race conditions and sequentially print partitions*/
    ...
    if (g_rank == 0) {
        ...
        /*
        printf("Time for Calculating %d Eigenvectors in CUDA: %.6f seconds\n", k_eigen, float(end1 - start1) / CLOCKS_PER_SEC);
